import type { ServiceTemplateContent } from "@lib/service-template";

export const curtainServiceContent: ServiceTemplateContent = {
  title: "Cortina de Vidrio | VIPAR S.A.",
  description:
    "Cortinas de vidrio en Paraguay para balcones, quinchos y frentes. Cerramientos corredizos con apertura prÃ¡ctica, visual limpia y fabricaciÃ³n a medida.",
  canonicalPath: "/servicios/cortinas",
  keywords:
    "cortina de vidrio paraguay, cerramiento de balcon, cortinas corredizas de vidrio, quinchos, cristal templado",
  whatsappMessage:
    "Hola, quiero cotizar una cortina de vidrio. Me comunico desde su pÃ¡gina web.",
  relatedWorksKey: "cortina",
  hero: {
    eyebrow: "Cortinas de vidrio",
    title: "Cortinas de vidrio para cerrar sin perder visual ni flexibilidad",
    description:
      "Sistemas corredizos de cristal templado para balcones, galerÃ­as, quinchos y frentes comerciales. Soluciones a medida para ganar protecciÃ³n sin bloquear luz ni amplitud.",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBEJiTJqpXth_MfquO_J3E-v_DDe0TyRJVLYYzzpH16eej51C8CFf80j_7iewEE7wYqXWcK7k16I9_f1JkfutQcc-s6C2LqxW64b5y9AKSTYoKtjdq8XFNxyvhdnqOgdfftQ8pVWflL4n9KbPUBhmwKhBLIf2ChNbt7mzufk7h46LxtScejB6AU8wTmvocHbchdvLVbqbCXRennx3hNVqFojmj3YAuXgFUCjF4tAzCATY4Mk9ADk9LJ8vlq5KOhXRBkjg7t_xFPG2O-",
    tags: ["Balcones", "Quinchos", "GalerÃ­as", "Cristal templado"],
    primaryCtaLabel: "Cotizar cortina",
  },
  problem: {
    title: "Â¿QuerÃ©s cerrar un espacio sin convertirlo en un ambiente oscuro o pesado?",
    description:
      "Cuando un balcÃ³n, quincho o galerÃ­a queda demasiado expuesto, el desafÃ­o es sumar protecciÃ³n climÃ¡tica sin perder visual, apertura ni lectura arquitectÃ³nica liviana.",
    cases: [
      {
        title: "ExposiciÃ³n al viento o lluvia",
        description: "Espacios abiertos que hoy no pueden usarse con comodidad todo el aÃ±o.",
      },
      {
        title: "Cierre visualmente pesado",
        description: "Soluciones opacas o con demasiada perfilerÃ­a que rompen la estÃ©tica del proyecto.",
      },
      {
        title: "Uso poco flexible",
        description: "Necesidad de abrir totalmente el frente cuando el clima acompaÃ±a.",
      },
    ],
  },
  solutions: {
    title: "Soluciones en cortinas de vidrio",
    description:
      "DiseÃ±amos cerramientos corredizos y plegables segÃºn el tipo de apertura, la exposiciÃ³n del lugar y el uso real del espacio.",
    items: [
      {
        title: "Cortina para balcÃ³n",
        description:
          "Sistema pensado para sumar protecciÃ³n y confort sin perder la vista ni la entrada de luz natural en departamentos y terrazas.",
        idealFor: ["Departamentos", "Balcones urbanos", "Frentes residenciales"],
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuBjz8d1csB8yEcfqQb_ovu_3-Cynl60L8-3sMkPHfv5hP49Ku2bipoK0Ao5hhtdhhQxD52tkpBW4WAV97xOHkRNNRveprZ5Wc6V18y20BJ-bAbFaoXUgez2-8MhHquQKrPUpcHHSD7QHStxo2dlwhWFXrkgnXIQQr9M0gPr1DD4lhVu4Aq_lyviD0CK88qBNgBmgzy-DUPQ7ATFJcXLGtyCzy2Lt5sl1Dt3gft181m2d46MpdNT5zfiCZkv1C3a3_uzGA440sIpl0ER",
      },
      {
        title: "Cortina para quincho o galerÃ­a",
        description:
          "Permite cerrar el espacio en dÃ­as de lluvia o viento y abrirlo por completo cuando se busca ventilaciÃ³n y continuidad exterior.",
        idealFor: ["Quinchos", "GalerÃ­as", "Ãreas sociales"],
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuC_AZXM1JTcYOaVXmvGw_Vze_hQmvNEAP7Zo4vt3xOhfE5BMcURTbfz5IBX6MZjzmCdyPiFH5AhiEcIDyf-6LKuIsTU6aGr8bdO_Aj-h3ztE6Xfjxg5Rhxw2PqcP0mlVVXD9bVR-OgNctN3iREvcS9-pBCWmFX-znQ1pp8tCNBVTqm_Mr9HyTM3e13VKT8J-4aOXKFRhKsM0qVnOxoB21qyLu-w2vrxqssp-ZlQj94-PR27YMLOi4VTs7wSfPC-AemqrbTdHRXveIBe",
      },
      {
        title: "Cortina para frente comercial",
        description:
          "Cerramiento de vidrio con apertura prÃ¡ctica para locales que necesitan visibilidad, seguridad y una fachada limpia.",
        idealFor: ["Locales", "Showrooms", "Frentes comerciales"],
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuDY8DHyzD0WpoxvZK76Fa9SlGDoVLOa6qsluCAGQbIZexUiW975Ex8qINya8aRnOTqvKr7gVDTHKDkYcxMpLBjXZr4AqmrM6dbM2Dhr2Qx1Gx9dCXTqJiqFByNfqndRHPOMqdmLpXfuP6V4lGSCfrcfxNGuW65C0lnnwqtrN75Kp0U2wkxUUrNnKONcdwFdxygnS2Gae0gdLW3-hEVNj1Gaen8ewXNichQJ-6FidWIlOOZo62gEe1_EVpYDiNdatETWrkCoM6DNUlia",
      },
    ],
  },
  selection: {
    title: "GuÃ­a de selecciÃ³n",
    description:
      "Definimos el sistema segÃºn apertura requerida, exposiciÃ³n al clima y nivel de uso del espacio.",
    items: [
      {
        title: "Apertura total",
        description: "Para espacios que deben poder liberarse casi por completo.",
        icon: "open_in_full",
      },
      {
        title: "Mayor protecciÃ³n",
        description: "Para zonas mÃ¡s expuestas a viento, lluvia o polvo.",
        icon: "shield",
      },
      {
        title: "Lectura liviana",
        description: "Para proyectos donde la visual limpia y la transparencia mandan.",
        icon: "visibility",
      },
    ],
  },
  applications: {
    title: "Aplicaciones por proyecto",
    description:
      "Adaptamos cada cortina a la geometrÃ­a del lugar y al modo en que ese espacio se usa todos los dÃ­as.",
    items: [
      {
        title: "Balcones",
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuBjz8d1csB8yEcfqQb_ovu_3-Cynl60L8-3sMkPHfv5hP49Ku2bipoK0Ao5hhtdhhQxD52tkpBW4WAV97xOHkRNNRveprZ5Wc6V18y20BJ-bAbFaoXUgez2-8MhHquQKrPUpcHHSD7QHStxo2dlwhWFXrkgnXIQQr9M0gPr1DD4lhVu4Aq_lyviD0CK88qBNgBmgzy-DUPQ7ATFJcXLGtyCzy2Lt5sl1Dt3gft181m2d46MpdNT5zfiCZkv1C3a3_uzGA440sIpl0ER",
      },
      {
        title: "Quinchos",
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuDnKq5W4WP4qsF3iHSyCQFjUIRCH0fdH8pMgC3B5iXdlu8K9mMks4b25sZocsFJwXdIgBkemMHmLT6S6Dk7LgKvnizVZDTEUwMmCV1tIm2xk-VbiLRLmqwgqmeCuaKm0kek2BaQt3MtViCpamY3w0-c8dFAGf_XYSAPlbtaTqW5EnarHHDoYLINOdfaumtlk_Q90LderZ9ttL1H7H9vX3GeCx-xQXcmADc2t5HL78PNdPw7bfEFE9OtfdFlKR9ABUccNL1OL0asjrqo",
      },
      {
        title: "GalerÃ­as",
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuD_D54uSg7RW_P3Ypm3tegIGkvM-RFMNrf_R9VNs1aMKXA1niBb3LMBs_v8dxtAQQLtHE-9F72hE7w4qSXokJeHSrrY9ocGlDiItocjZC0QPmwVVa3NR7vHL7kHD9er3DuFFTFIhlnmxs5S5Vb_ciYTTMw6wdtXHhdiKMpvu6QmLqlk7eC58pofXKz70WEpfMSTGeTGij_UZae0sITPacXNhLUJNba0qXSXvbUVQxZgR2hXjSdoXY7MKqN6gIAet1NbsiLkIlcZlups",
      },
      {
        title: "Locales",
        image:
          "https://lh3.googleusercontent.com/aida-public/AB6AXuDY8DHyzD0WpoxvZK76Fa9SlGDoVLOa6qsluCAGQbIZexUiW975Ex8qINya8aRnOTqvKr7gVDTHKDkYcxMpLBjXZr4AqmrM6dbM2Dhr2Qx1Gx9dCXTqJiqFByNfqndRHPOMqdmLpXfuP6V4lGSCfrcfxNGuW65C0lnnwqtrN75Kp0U2wkxUUrNnKONcdwFdxygnS2Gae0gdLW3-hEVNj1Gaen8ewXNichQJ-6FidWIlOOZo62gEe1_EVpYDiNdatETWrkCoM6DNUlia",
      },
    ],
  },
  process: {
    title: "Proceso de cotizaciÃ³n",
    description:
      "Analizamos apertura, dimensiones y exposiciÃ³n para definir un sistema que funcione bien en uso real.",
    steps: [
      { label: "EnviÃ¡ fotos" },
      { label: "Tomamos medidas" },
      { label: "Definimos apertura" },
      { label: "Proponemos sistema" },
      { label: "Coordinamos instalaciÃ³n" },
    ],
  },
  trust: {
    title: "Confianza e industria nacional",
    description:
      "VIPAR S.A. fabrica e instala soluciones en vidrio y aluminio con criterio tÃ©cnico, respaldo local y seguimiento real del proyecto desde la mediciÃ³n hasta la colocaciÃ³n final.",
    locationLabel: "UbicaciÃƒÂ³n operativa: Ãƒâ€˜emby, Paraguay",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuD5SUTj8uLe8_yoSaL6zwpOvBISR9f56pg4QTpMM22Ub8JaAH8Rrxgp9U827sU_lm7JW9RlRLm1JN2f9D1RPBSsohyC7qdOAW5bIqkVBEDWbArcN7ub3_sTK4NbKGZmJx5CYzqEYraU9f1UMCrtdgsiKJB8ShPaeOBlYtptqtOC6T0Mj07X5Pt1qNu5gbHlBOMTKEfGb2VTqHk6vYYCF1Jl14wM6s9jNiquK9Lwj19QCWxsY9MzJcgHeGZPIveNOgAa57_9Xr9",
  },
  faqs: [
    {
      question: "Â¿Las cortinas de vidrio cierran completamente el paso del agua?",
      answer:
        "Depende del sistema, del nivel de exposiciÃ³n y de la geometrÃ­a del lugar. DiseÃ±amos la soluciÃ³n buscando el mejor equilibrio entre estanqueidad, apertura y liviandad visual.",
    },
    {
      question: "Â¿Se pueden abrir por completo?",
      answer:
        "SÃ­, segÃºn el sistema elegido. Existen configuraciones que permiten apilar o desplazar las hojas para liberar gran parte del vano.",
    },
    {
      question: "Â¿Sirven para balcones y quinchos?",
      answer:
        "SÃ­. Son dos de las aplicaciones mÃ¡s comunes porque permiten ganar protecciÃ³n climÃ¡tica y extender el uso del espacio sin cerrar visualmente el ambiente.",
    },
  ],
};



